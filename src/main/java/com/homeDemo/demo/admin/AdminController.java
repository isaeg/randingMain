...
//        return "content/admin/qaTable";
    ...
    @GetMapping("/teamSales")
    public String goTeamSaless() {
        //  model.addAttribute("hi", "world");
        return "content/admin/teamSalesTable";
    }
    ...
    @PostMapping("/searchTeamList.ajax")
    @ResponseBody
    public  ModelAndView searchTeamList(Model model , SalesHistoryVO param) {
        ModelAndView mv = new ModelAndView("jsonView");
        int count  = salesService.saleTeamCount(param);
        List<SalesHistoryVO> saleList= salesService.getTeamSaleList(param ,count);
        Pagenation pagination = new Pagenation(count, param);
        mv.addObject("result",saleList );
        mv.addObject("params", pagination);
        return mv;
